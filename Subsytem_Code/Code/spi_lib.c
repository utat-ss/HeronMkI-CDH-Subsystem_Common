...
{
	uint8_t* reg_ptr;
	uint8_t timeout = SPI_TIMEOUT;		// Decrease the SPI timeout to try to escape the program init loop
	reg_ptr = SPDR_BASE;
	// Commence the SPI message.
	*reg_ptr = message;
	reg_ptr = SPSR_BASE;
...
void spi_send_shunt_dpot_value(uint8_t message)
{
	...
	*reg_ptr |= (0b01000000);
	...
	// Set the wiper
	spi_result = spi_transfer3(0x00);
	delay_cycles(8); // This NEEDS to be here! Why? IDK
	spi_result = spi_transfer3(message);
	spi_result = spi_transfer3(message);
	...
	spi_result = spi_transfer3(0x20);
	delay_cycles(8); // This NEEDS to be here! Why? IDK
	spi_result = spi_transfer3(0xFF);
	spi_result = spi_transfer3(0xFF);
